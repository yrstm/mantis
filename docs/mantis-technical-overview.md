...
- **Unit/behavioral tests** (`test.js`, run with `node test.js`): 99 tests covering scope selection, chrome rejection, dedup, block/section/citation construction, image and table extraction, inline-run invariants, Markdown rendering, budgeting, frontmatter, the image pipeline, and the bookmarklet delivery flow.
