...
- **Word-level scorecard** (`benchmark.js`, same corpus): every fixture carries a **gold annotation** (CSS selectors resolving to the true content elements, with optional exclusions), and the harness computes word-level **precision / recall / F1** of the extracted blocks+tables against that gold, reported per fixture and **aggregated per page type** (article, docs, forum, social, landing, linklist, newsletter, recipe, app-shell), plus per-fixture extraction latency and the confidence score alongside the achieved F1. Each fixture gates on a minimum F1 (default 0.98), so quality regressions on any page type fail the build instead of hiding in an aggregate. `npm run scorecard` emits the same data as JSON for cross-tool comparison.
...
- **Unit/behavioral tests** (`test.js`, run with `node test.js`): 117 tests covering scope selection, chrome rejection, dedup, block/section/citation construction, image and table extraction, inline-run invariants, Markdown rendering, budgeting, frontmatter, the image pipeline, and the bookmarklet delivery flow.
